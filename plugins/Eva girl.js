...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('helena') && conf.TALKING_HELENA !== 'true') {        
        ...
        var finm = message.message.replace('helena', '').replace(' ', '')   
...
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.TALKING_HELENA == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
Asena.addCommand({ pattern: 'helena ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.helena on / off' }, (async (message, match) => {
    var pinky_status = `${conf.TALKING_HELENA}`
    if (match[1] == 'on') {
